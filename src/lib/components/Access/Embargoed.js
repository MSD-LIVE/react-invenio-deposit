...
export class Embargoed {
  ...
  renderMessageSection() {
    ...
    const text = (
      <Trans
        defaults="On <bold>{{ date }}</bold> the record and the files will automatically be made publicly accessible. Until then, the record and the files can <bold>only</bold> be accessed by <bold>users on the same project team</bold>."
        values={{ date: fmtDate }}
        components={{ bold: <b /> }}
      />
    );
